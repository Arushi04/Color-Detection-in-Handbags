...
def check_wishlist_fav(url):
    ...
    if wishlist_soup == "Timeout error":
        logging.info(url, "Wishlist Page not loaded")
        return 3, None

    fav_soup = get_sel_response(url + '#fav')
    if fav_soup == "Timeout error":
        logging.info(url, "Favorite Page not loaded")
        return 3, None
...
def check_followers_count(soup):
    ...
    logging.info(f"follower : {follower} following :  {following} ")
...
def main(args):
    ...
    if os.path.isfile(args.outputfile):
        ...
        if os.path.isfile(args.inputfile):   # Else read from the main input file
            ...
            col_names = list(df.columns)
            logging.info("Col names : ", col_names)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--logfile", type=str, default='info.log', help='log file name')
